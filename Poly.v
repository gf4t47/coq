...
Lemma rev_snoc_involutive : forall X : Type, forall n : X, forall l : list X,
...
Fixpoint split {X Y : Type} (lp : list (X * Y)) 
           : ((list X) * (list Y)) :=
match lp with
  |nil => ([],[])
  |(x,y)::lp' => ((cons x (fst (split lp'))),(cons y (snd (split lp'))))
end.
...
Definition negb_test {X : Type} (test : X -> bool) (x : X) : bool :=
  negb (test x).  
Definition partition {X : Type} (test : X -> bool) (l : list X)
                     : list X * list X :=
  ((filter test l), (filter (negb_test test) l)).

Definition negb_test1 {X : Type} (x : X) (test : X -> bool) : bool :=
  negb (test x).  
Definition partition1 {X : Type} (test : X -> bool) (l : list X)
                     : list X * list X :=
  ((filter test l), (filter (fun x => negb_test1 x test) l)).

Fixpoint partition2 {X : Type} (test : X -> bool) (l : list X)
                     : list X * list X :=
  match l with
    | nil => (nil,nil)
    | x::l' => if (test x) then ((cons x (fst (partition test l'))),(snd (partition test l')))
                           else ((fst (partition test l')),(cons x (snd (partition test l'))))
  end.                          
...
Example test_partition3: partition1 oddb [1,2,3,4,5] = ([1,3,5], [2,4]).
Proof. reflexivity.  Qed.
Example test_partition4: partition1 (fun x => false) [5,9,0] = ([], [5,9,0]).
Proof. reflexivity.  Qed.

Example test_partition5: partition2 oddb [1,2,3,4,5] = ([1,3,5], [2,4]).
Proof. reflexivity.  Qed.
Example test_partition6: partition2 (fun x => false) [5,9,0] = ([], [5,9,0]).
Proof. reflexivity.  Qed.
(** [] *)
...
Theorem map_snoc : forall (X Y : Type) (x : X) (f : X -> Y) (l : list X),
...
Definition option_map_explicit (X : Type) (Y : Type) (f : X -> Y) (xo : option X)
                      : option Y :=
  match xo with
    | None => None
    | Some x => Some (f x)
...
Example test_option_map1 : option_map_explicit nat nat (fun x:nat => x + 1) (index 0 [4,5,6,7])  = Some 5.
Proof. reflexivity.  Qed.
Example test_option_map2 : option_map_explicit (list nat) (list nat) (fun l => map (fun x : nat => x + 1) l) (index  1 [[1],[2]]) = Some [3].
Proof. reflexivity.  Qed.
...
Check (fold (fun n : nat => (fun b: bool => andb b (evenb n)))).
Check (fold (fun (n : nat) (b : bool) => andb b (evenb n))).
(* ###################################################### *)
...
Theorem fold_map_map : forall X Y (l : list X)(f : X -> Y),
...
Theorem index_last : forall X l n, 
