...
Fixpoint plus_r (n : nat) (m : nat) : nat :=
  match m with
    | O => n
    | S m' => S (plus_r n m')
  end.

Theorem plus_r_O_n : forall n : nat, plus_r 0 n = n.
Proof.  Admitted.

Theorem plus_r_n_O : forall n : nat, plus_r n 0 = n.
Proof.  reflexivity.  Qed.

Eval simpl in (plus 0 8).
Eval simpl in (plus 8 0).
Eval simpl in (plus_r 0 8).
Eval simpl in (plus_r 8 0).

(** **** answer: 
   I think the difference is based on the definition of plus function, 
   the standard version is recursive on the left argument, 
   which makes it get the value of plus fuction to equal right argument immediately
   by O => m when left argument is O.
   And while left argument is not O (no matter what right argument is O or not) 
   the plus function return a recursive procedure the value of which can not be evaluated now since left argument is not bind yet. 
   In the self definition of right argument recursive version we can see the opposite situation. 
   It seems that Theorem-Proof only return the body of right side of : but do not eval it further. 
*)

End Playground_exercise.
...
Proof.
  Lemma andb_eq_rt :
     forall (a : bool),andb a true = a.    
  Proof.
     intros a.
     destruct a.
     reflexivity. reflexivity.  Qed.
  Lemma andb_eq_rf :
     forall (a : bool),andb a false = false.    
  Proof.
     intros a.
     destruct a.
     reflexivity. reflexivity.  Qed.
  Lemma orb_eq_rf :
     forall (a : bool), orb a false = a.
     intros a.
     destruct a.
     reflexivity. reflexivity.  Qed.
  Lemma orb_eq_rt :
     forall (a : bool), orb a true = true.
     intros a.
     destruct a.
     reflexivity. reflexivity.  Qed.
  intros b c.
...
Fixpoint plus_1bin (n : bin) : bin :=
  match n with
    | o => add o
    | dub n' => add n
    | add n' => dub (plus_1bin n')
  end.

Fixpoint bin_nat (n : bin) : nat :=
  match n with
    | o => 0
    | dub n' => (bin_nat n') * 2
    | add n' => (bin_nat n') + 1
...
(** **** Fixpoint plus_bin (n m : bin) : bin :=
  match n with
    ...
    | dub n' => match m with
                  | o => n
                  | dub m' => dub (plus_bin n' m')
                  | add m' => plus_1bin (plus_bin n m')
                end
    | add n' => plus_1bin (plus_bin n' m)
  end. **)

Eval simpl in (bin_nat (dub (add (add (add o))))).
Eval simpl in (bin_nat (dub (dub (dub o)))).
Eval simpl in (bin_nat (dub (dub (add o)))).
Eval simpl in (bin_nat (dub (add (dub o)))).
Eval simpl in (bin_nat (add (dub (add (dub (dub (add o))))))).

Eval simpl in (bin_nat (plus_1bin (dub (add (add (add o)))))).
Eval simpl in (bin_nat (plus_1bin (dub (add (dub (add o)))))).
Eval simpl in (bin_nat (plus_1bin o)).
Eval simpl in (bin_nat (plus_1bin (add o))).
Eval simpl in (bin_nat (plus_1bin (dub (add o)))).
Eval simpl in (bin_nat (plus_1bin (dub (add (add o))))).

Eval simpl in ((bin_nat (dub o)) + (bin_nat (dub o))).
Eval simpl in ((bin_nat (add o)) + (bin_nat (dub o))).
Eval simpl in ((bin_nat (add o)) + (bin_nat (dub (add o)))).
...
(** **** Answer
Fixpoint plus_both (n : nat) (m : nat) : nat :=
  match n, m with
  | O , _  => m
  | _ , O  => n
  | _ , S m' => S (plus_both n m')
  | S n' , _ => S (plus_both n' m)
  end.
*)
...
(* $Date: 2013-02-18 09:09:09 -0500 (Mon, 18 Feb 2013) $ *)
