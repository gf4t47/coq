...
(** **** answer: 
    induction automaticly gives us a Induction Hypothesis which make us to prove
    P(S n') easily. destruct just divid the goal into sub cases.
*)
(** [] *)
...
Lemma mult_plus1 : forall k n : nat,
  k * (n + 1) = k * n + k.
Proof.
  intros k n.
  induction k as [|k'].
...
Proof.
  ...
  rewrite -> mult_0_r.
  reflexivity.
  assert (A1 : (n + m) * S p' = S p' * (n + m)).
    Case "Proof of A1".
    rewrite -> mult_comm. reflexivity.
  assert (A2 : n * S p' = S p' * n).
    Case "Proof of A2".
    rewrite -> mult_comm. reflexivity.
  assert (A3 : m * S p' = S p' * m).
    Case "Proof of A3".
    rewrite -> mult_comm. reflexivity.
  rewrite -> A1.
  rewrite -> A2.
...
Fixpoint plus1bin (n : bin) : bin :=
  match n with
    | o => add o
    | dub n' => add n'
    | add n' => dub (plus1bin n')
  end.
...
Eval simpl in (bin2nat o). (** 0 **)
Eval simpl in (bin2nat (add o)). (** 1 **)
Eval simpl in (bin2nat (dub (add o))). (** 2 **)
Eval simpl in (bin2nat (add (add o))). (** 3 **)
Eval simpl in (bin2nat (dub (dub (add o)))). (**4 **)
Eval simpl in (bin2nat (add (dub (add o)))). (** 5 **)
Eval simpl in (bin2nat (dub (add (add o)))). (** 6 **)
Eval simpl in (bin2nat (add (add (add o)))). (** 7 **)
Eval simpl in (bin2nat (dub (dub (dub (add o))))). (** 8 **)
Eval simpl in (bin2nat (add (dub (dub (add o))))). (** 9 **)
Eval simpl in (bin2nat (dub (add (dub (add o))))). (** 10 **)
Eval simpl in (bin2nat (add (add (dub (add o))))). (** 11 **)
Eval simpl in (bin2nat (dub (dub (add (add o))))). (** 12 **)
Eval simpl in (bin2nat (add (dub (add (add o))))). (** 13 **)
Eval simpl in (bin2nat (dub (add (add (add o))))). (** 14 **)
Eval simpl in (bin2nat (add (add (add (add o))))). (** 15 **)
Eval simpl in (bin2nat (dub (dub (dub (dub (add o)))))). (** 16 **)


Eval simpl in (bin2nat (plus1bin o)). (** 1 **)
Eval simpl in (bin2nat (plus1bin (add o))). (** 2 **)
Eval simpl in (bin2nat (plus1bin (dub (add o)))). (** 3 **)
Eval simpl in (bin2nat (plus1bin (add (add o)))). (** 4 **)
Eval simpl in (bin2nat (plus1bin (dub (dub (add o))))). (** 5 **)
Eval simpl in (bin2nat (plus1bin (add (dub (add o))))). (** 6 **)
Eval simpl in (bin2nat (plus1bin (dub (add (add o))))). (** 7 **)
Eval simpl in (bin2nat (plus1bin (add (add (add o))))). (** 8 **)
Eval simpl in (bin2nat (plus1bin (dub (dub (dub (add o)))))). (** 9 **)
Eval simpl in (bin2nat (plus1bin (add (dub (dub (add o)))))). (** 10 **)
Eval simpl in (bin2nat (plus1bin (dub (add (dub (add o)))))). (** 11 **)
Eval simpl in (bin2nat (plus1bin (add (add (dub (add o)))))). (** 12 **)
Eval simpl in (bin2nat (plus1bin (dub (dub (add (add o)))))). (** 13 **)
Eval simpl in (bin2nat (plus1bin (add (dub (add (add o)))))). (** 14 **)
Eval simpl in (bin2nat (plus1bin (dub (add (add (add o)))))). (** 15 **)
Eval simpl in (bin2nat (plus1bin (add (add (add (add o)))))). (** 16 **)
Eval simpl in (bin2nat (plus1bin (dub (dub (dub (dub (add o))))))). (** 17 **)

Eval simpl in ((bin2nat (dub o)) + (bin2nat (dub o))).
Eval simpl in ((bin2nat (add o)) + (bin2nat (dub o))).
Eval simpl in ((bin2nat (add o)) + (bin2nat (dub (add o)))).

Theorem bin_plus1_trans_comm : forall n : bin, 
  bin2nat (plus1bin n) = (bin2nat n) + 1.
Proof.
  intros n.
  induction n as [| dn' | an' ].
  Case "n = 0".
  reflexivity.
  Case "n = dub dn'".
  simpl. reflexivity.
...
Fixpoint plus_bin (n m : bin) : bin :=
  match n with
    ...
    | dub n' => match m with
                  ...
                  | add m' => plus1bin (dub (plus_bin n' m'))
                end
    | add n' => match m with
                  | o => n
                  | dub m' => plus1bin (dub (plus_bin n' m'))
                  | add m' => plus1bin (plus1bin (dub (plus_bin n' m')))
                end
  end.

Theorem double_plus_ref_1st : forall n : bin,
  plus_bin n n = dub n.
Proof.
  intros n.
  destruct n.
  simpl.
  Admitted.
(** **** I can not even prove the sub theorem above, because I am stuck by (dub o = o).
    I guess the problem is that there are more than one representation for a sole bin number.
**)

(** **** answer c *)
Definition bin_dub (n: bin): bin :=
  match n with
    | o => o
    | dub x => dub (dub x)
    | add x => dub (add x)
...
Fixpoint normalize (n : bin) : bin :=
  match n with
    | o => o
    | dub n' => match normalize n' with
                  | o => o
                  | dub n'' => dub (dub n'')
                  | add n'' => dub (add n'')
                  end
    | add n' => add (normalize n')
  end.

Eval simpl in (normalize o).
Eval simpl in (normalize (add o)).
Eval simpl in (normalize (dub (add o))).
Eval simpl in (normalize (add (dub (add o)))).
Eval simpl in (normalize (dub (dub (add o)))).
Eval simpl in (normalize (dub (dub (dub (add o))))).
Eval simpl in (normalize (add (dub (dub (add o))))).

Eval simpl in (normalize (dub o)).
Eval simpl in (normalize (dub (dub o))).
Eval simpl in (normalize (dub (dub (dub o)))).
Eval simpl in (normalize (dub (dub (dub (dub o))))).

Eval simpl in (normalize (add (dub (dub (dub o))))).
Eval simpl in (normalize (dub (dub (add (dub o))))).
Eval simpl in (normalize (dub (add (dub (dub o))))).
Eval simpl in (normalize (dub (dub (add (dub o))))).
Eval simpl in (normalize (add (dub (dub (dub (dub o)))))).

Lemma plus2_double: forall n : bin, 
  plus1bin (plus1bin (bin_dub n)) = bin_dub (plus1bin n).
Proof.
  intros n.  
  destruct n.
  Case "n = o".
...
Lemma dub_plus_ref:  forall n : nat,
    nat2bin (n + n) = bin_dub (nat2bin n).
Proof.
  intros n.
  induction n.
  Case "n = 0".
  ...
  simpl.
  rewrite IHn.
  rewrite plus2_double.
  reflexivity. Qed.

Lemma add_plus_ref1:  forall n : nat,
  nat2bin (n + n + 1) = add (nat2bin n).
Proof.
  intros n.
  induction n.
  Case "n = 0".
...
Theorem bin_nat_bin : forall n : bin,
...
Proof.
  ...
  rewrite -> mult_plus1.
  rewrite -> mult_1.
  rewrite -> add_plus_ref1.
  rewrite -> IHan'.
  reflexivity. Qed.

End Playground_binary.
(** [] *)
...
(** **** answer
     Theorem: Addition is commutative that
     For any [n], [m],  n + m = m + n.
     _Proof_: By induction on [n].
     ...
     - Next, suppose [n = S n'], where n' + m = m + n'.
     We must show (S n') + m = m + S n'
     By the definition of [+], this follows from
        S n' + m = S (n' + m)
     By the induction hypothesis.
        S(n' + m) = S (m + n')
     By the definition of [+] ans S, 
        S (m + n') = m + S n'.
**)
...
(** **** Write an informal proof of the following theorem, using the
    ...
    - Next, suppose [n = S n'], where true = beq_nat n' n'.
    We must show true = beq_nat (S n') (S n').
    beq_nat (S n') (S n') = beq_nat n' n',
    This follows directly from the definition of [beq_nat].
    By the induction hypothesis, true = beq_nat n' n'.
    that true = beq_nat n' n' = beq_nat (S n') (S n').
